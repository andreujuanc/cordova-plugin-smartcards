...
namespace Pcsc
{
    public sealed class Reader
    {
        ...
        public async void Init()
        {
          ...
          //  proximityDevice = ProximityDevice.GetDefault();
           ...
           //     {
            ...
            {
                LogMessage("Your NFC proximity setting is turned off, you will be taken to the NFC proximity control panel to turn it on");
        ...
        public IAsyncOperation<CardInfo> GetCurrentCardTagId()
        {
            return Task.Run<CardInfo>(async () =>
            {
                var deviceInfo = await SmartCardReaderUtils.GetFirstSmartCardReaderInfo(SmartCardReaderKind.Nfc);
                if (deviceInfo != null)
                {
                    m_cardReader = await SmartCardReader.FromIdAsync(deviceInfo.Id);
                    var result = await m_cardReader.FindAllCardsAsync();
                    if (result != null || result.Count > 0)
                    {
                        return await HandleCard(result.First());
                    }
                }
                return null;
            }).AsAsyncOperation<CardInfo>();                 
        }
        ...
        private async Task<CardInfo> HandleCard(SmartCard card)
        {
            try
            {
                // Clear the messages
                ...
                using (SmartCardConnection connection = await card.ConnectAsync())
                {
                    ...
                    info.DeviceClass = cardIdentification.PcscDeviceClass.ToString();
                    info.CardName = cardIdentification.PcscCardName.ToString();
                        ...
                        || cardIdentification.PcscCardName == Pcsc.CardName.MifareUltralightEV1))
                    {
                        ...
                        info.UID = BitConverter.ToString(responseUid).Replace("-", string.Empty);
                    }
